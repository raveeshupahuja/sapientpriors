...
app.post("/api/contact", async (req, res) => {
  ...
  console.log('Contact form received:', { name, email, company, useCase, subject: subject || 'Contact Form Submission' });
    ...
    try {
      await resend.emails.send({
        ...
        html: `
          ...
          ${message ? `<h3>Additional Information:</h3><p style="white-space: pre-wrap;">${message}</p>` : ''}
          <hr>
...
app.post("/api/careers/apply", upload.single('resume'), async (req: any, res) => {
  ...
  console.log('Career application received:', {
    name, email, phone, linkedin, role, experience,
  ...
  if (resend) {
    try {
      await resend.emails.send({
        from: 'SapientPriors Careers <onboarding@resend.dev>',
        to: 'raveeshupahuja@gmail.com',
